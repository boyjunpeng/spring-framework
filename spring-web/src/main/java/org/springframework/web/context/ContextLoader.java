...
public class ContextLoader {
		...
		try {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					configureAndRefreshWebApplicationContext(cwac, servletContext);  //初始化IOC容器
				}
			}
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		Class<?> contextClass = determineContextClass(sc);//根据Servlet中的配置决定用那个IOC容器
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		else {//spring-web模块下根路径中ContextLoader.properties配置文件中的配置信息
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		String configLocationParam = sc.getInitParameter(CONFIG_LOCATION_PARAM);//读取web.xml中的init-param 参数 contextConfigLocation
